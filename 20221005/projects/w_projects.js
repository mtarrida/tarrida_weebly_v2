...
export function allItemsList() {
    ...
    let sectionDiv = $('.wsite-section-wrap:contains("general_title")')
    // Array.from(document.getElementsByClassName('wsite-section-wrap'))
    // [1]; // recorda que hi ha una seccio 0...
    // console.log('sectionDiv:', sectionDiv)

    let clone_ = sectionDiv.clone()
    // .cloneNode(true);
    sectionDiv.remove();

    // 2on agafo la ruta on hi tinc els scripts
    ...
    projectsJson
        .filter(Z => Z.display)
        .forEach(Z => {
            let newSection = clone_.clone();
            newSection.find('h2:contains("general_title")').text(Z.title)
            newSection.find('span:contains("general_text")').text(Z.text)
            console.log('aaa');
            newSection.find('img[alt="general_picture"]')
                // .attr("src", Z.img_1_src)
                .attr("src", Z.img_1_src.replace("@imgServerPath@", customScriptsPath))



            newSection.appendTo($('#wsite-content'))
